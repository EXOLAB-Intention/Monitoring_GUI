...
class Model3DViewer(QGLWidget):
    ...
    def reset_view(self):
        """Reset the 3D view to its default orientation and pose."""
        self.rotation_x = 0
        self.rotation_y = 0
        self.rotation_z = 0
...
class Model3DWidget(QWidget):
    ...
    # Méthodes d'interface utilisées par dashboard_app.py et dashboard_app_back.py
    ...
    def start_tpose_calibration(self):
        if hasattr(self.model_viewer, 'start_calibration'):
            return self.model_viewer.start_calibration(3)
        return False
    
    def stop_tpose_calibration(self):
        if hasattr(self.model_viewer, 'finish_calibration'):
            return self.model_viewer.finish_calibration()
        return False
    
    def reset_tpose_calibration(self):
        if hasattr(self.model_viewer, 'reset_calibration'):
            return self.model_viewer.reset_calibration()
        return False

    def get_calibration_status(self):
        if hasattr(self.model_viewer, 'get_calibration_status'):
            return self.model_viewer.get_calibration_status()
        return { 'status_text': 'Status unknown', 'progress': 0, 'complete': False, 'mode': False }

    # NEW METHOD: Expose reset_view to DashboardApp
    def reset_view(self):
    ...
    # Ajouter les méthodes manquantes pour la calibration T-pose
    def start_tpose_calibration(self):
        """Démarre la calibration T-pose."""
        if hasattr(self.model_viewer, 'start_calibration'):
            return self.model_viewer.start_calibration(3)  # 3 secondes de durée
        return False
    ...
    def get_calibration_status(self):
        """Récupère le statut de calibration."""
        if hasattr(self.model_viewer, 'get_calibration_status'):
            return self.model_viewer.get_calibration_status()
        # Retourne un objet compatible avec l'interface attendue
        return {
