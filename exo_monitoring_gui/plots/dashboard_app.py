...
from plots.back.dashboard_app_back import DashboardAppBack
...
class DashboardApp(QMainWindow):
    def __init__(self, subject_file=None, parent_revi=None, file_list=None):
        ...
        self._plot_update_interval_ms = 50
        self.setStyleSheet("""
            QMainWindow, QDialog {
    ...
    def init_ui(self):
        ...
        self.calibration_status_timer.start(500)
    ...
    def toggle_motion_prediction(self):
        try:
            ...
            if not hasattr(self, 'model_3d_widget') or not self.model_3d_widget:
                raise AttributeError("3D model widget is not available")

            if not hasattr(self.model_3d_widget, 'toggle_motion_prediction'):
                raise AttributeError("3D model doesn't support motion prediction")
    ...
    def start_tpose_calibration(self):
        ...
        try:
            ...
            if not hasattr(self.model_3d_widget, 'start_tpose_calibration'):
                if hasattr(self.model_3d_widget.model_viewer, 'start_calibration'):
                    print("[UI_DEBUG] Using model_viewer.start_calibration instead")
                    result = self.model_3d_widget.model_viewer.start_calibration(3)
                else:
                    raise AttributeError("3D model doesn't support T-pose calibration")
            else:
    ...
    def stop_tpose_calibration(self):
        ...
        try:
            ...
            if hasattr(self.model_3d_widget, 'stop_tpose_calibration'):
                result = self.model_3d_widget.stop_tpose_calibration()
            elif hasattr(self.model_3d_widget.model_viewer, 'finish_calibration'):
                result = self.model_3d_widget.model_viewer.finish_calibration()
            else:
                raise AttributeError("3D model doesn't support stopping T-pose calibration")
    ...
    def reset_tpose_calibration(self):
        ...
        try:
            ...
            if hasattr(self.model_3d_widget, 'reset_tpose_calibration'):
                self.model_3d_widget.reset_tpose_calibration()
            elif hasattr(self.model_3d_widget.model_viewer, 'reset_calibration'):
                self.model_3d_widget.model_viewer.reset_calibration()
            elif hasattr(self.model_3d_widget, 'reset_calibration'):
                self.model_3d_widget.reset_calibration()
            else:
                print("[WARNING] No compatible reset calibration method found")
    ...
    def on_sensor_clicked(self, item, column):
        """Handle click on a sensor item in the tree."""
        try:
